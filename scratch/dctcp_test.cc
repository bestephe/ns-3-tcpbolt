...
{
  LogComponentEnable("DctcpTest", LOG_LEVEL_ALL);
  LogComponentEnableAll(LOG_PREFIX_TIME);
  LogComponentEnableAll(LOG_PREFIX_FUNC);

  //GlobalValue::Bind ("ChecksumEnabled", BooleanValue (true));
  //GlobalValue::Bind ("SchedulerType", TypeIdValue (CalendarScheduler::GetTypeId ())); //XXX: Crazy debugging idea
  Packet::EnablePrinting ();

  bool tracing = false;
  uint32_t maxBytes = 0;
  uint32_t threshold = 50e3;
...
//
  ...
  Config::SetDefault ("ns3::TcpSocket::SndBufSize", UintegerValue (1e9)); //Large value
  Config::SetDefault ("ns3::TcpSocket::RcvBufSize", UintegerValue (1e9)); //Large value
...
//
  ...
  pointToPoint.SetQueue ("ns3::SimpleRedEcnQueue",
  ...
  pointToPoint.EnablePcapAll ("DctcpTest");
