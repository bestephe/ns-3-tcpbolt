...
//	LogComponentPrintList();
	...
	double linkDelay = 20e-6;	// Link delay in seconds (5ns = 1m wire)
        uint32_t queueBytes = 22.5e3;      // Number of bytes per queue port
	unsigned size = 3;		// Fat tree size
        double loadFactor = 0.1;         // The load factor the incasts
        uint32_t initCwnd = 200;         // TCP Initial Congestion Window
        double minRto = 600e-6;        // Minimum RTO
        //double minRto = -1;
	...
	Config::SetDefault ("ns3::HashRouting::EnableReroute", BooleanValue (false));
        ...
        //Config::SetDefault ("ns3::RttEstimator::InitialEstimation", TimeValue (Seconds (minRto))); //TODO: What should the initial estimate be?
        if (minRto > 0) {
            Config::SetDefault ("ns3::RttEstimator::MinRTO", TimeValue (Seconds (minRto)));
            Config::SetDefault ("ns3::TcpL4Protocol::RttEstimatorType", TypeIdValue (RttStatic::GetTypeId ()));
        } else { NS_ASSERT (false); }

        /* Set the transport protocol */
        Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TcpNewReno::GetTypeId ()));
        std::string queueType = "ns3::PriorityQueue";
	...
	const unsigned numHosts = size * size * size * 2;
        if (star) {
            ...
            //XXX: This seems reasonable for pFabric
	...
	NS_ASSERT_MSG(hostIfs.GetN() == numHosts,
        ...
        //XXX: Only one destination for testing
        //NodeContainer dsts = NodeContainer (hosts.Get (0));
	...
	for (unsigned i = 0; i < numHosts; i++) {
        ...
        //XXX:
        //remoteHostAddrs.push_back (InetSocketAddress (hostAddrs[0], port));
        ObjectFactory bulkSendFactory;
        bulkSendFactory.SetTypeId ("ns3::PriorityPersistentBulkSendApplication");
        bulkSendFactory.Set ("SendSize", UintegerValue (4192));
        bulkSendFactory.Set ("Protocol", StringValue (socketSpec));
        ApplicationContainer apps;
	for (unsigned i = 0; i < numHosts; i++) {
            Ptr<PriorityPersistentBulkSendApplication> bulkSend = bulkSendFactory.Create<PriorityPersistentBulkSendApplication> ();
...
//        LogComponentEnable("TcpSocketBase", LOG_LEVEL_ALL);
//        LogComponentEnable("PriorityIncastAggregator", LOG_LEVEL_ALL);
//        LogComponentEnable("PriorityIncastSender", LOG_LEVEL_ALL);
