...
NS_OBJECT_ENSURE_REGISTERED (RttStatic);

TypeId 
RttStatic::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RttStatic")
    .SetParent<RttEstimator> ()
    .AddConstructor<RttStatic> ()
  ;
  return tid;
}
...
RttStatic::RttStatic (const RttStatic& c)
  : RttEstimator (c)
{
  NS_LOG_FUNCTION (this);
}

void RttStatic::Measurement (Time m)
{
}

Time RttStatic::RetransmitTimeout ()
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("RetransmitTimeout:  return " << m_minRto.GetSeconds ());
  return (m_minRto);  
}

Ptr<RttEstimator> RttStatic::Copy () const
{
  NS_LOG_FUNCTION (this);
  return CopyObject<RttStatic> (this);
}

void RttStatic::Reset ()
{ 
  NS_LOG_FUNCTION (this);
  // Reset to initial state
  RttEstimator::Reset ();
