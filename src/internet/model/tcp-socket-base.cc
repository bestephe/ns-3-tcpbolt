...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("Priority", "The priority of the socket",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&TcpSocketBase::m_priority),
                           ...
                           Address saddr, Address daddr, Ptr<NetDevice> oif)
{
  if (m_priority > -1)
     {
      PriorityTag priTag;
      double actualPriority = m_priority - m_ackdBytes;
#if 0
      if (outgoing.GetFlags () & TcpHeader::ACK)
         { // If sending an ACK, use lowest priority
            actualPriority = 0;
         }
#endif
      priTag.SetPriority (actualPriority);
      packet->AddPacketTag (priTag);
      NS_LOG_LOGIC ("TcpSocketBase: Sending packet with priority: " << actualPriority);
      //NS_LOG_UNCOND ("TcpSocketBase: Sending packet with priority: " << actualPriority << " (" << m_priority << " - " << m_ackdBytes);
     }
   else
     {
      NS_LOG_LOGIC ("TcpSocketBase: Sending packet with no priority");
      //NS_LOG_UNCOND ("TcpSocketBase: Sending packet with no priority");
     }
